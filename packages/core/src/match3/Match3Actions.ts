...
export class Match3Actions {
  ...
  public async actionMove(from: Match3Position, to: Match3Position) {
    ...
    log('[Match3] ACTION! Move:', from, 'to:', to);
    await this.swapPieces(pieceA, pieceB);
    this.match3.process.start();
  }

  /**
   * Tap action — disabled, special pieces must be activated by swapping, not tapping
   */
  public async actionTap(_position: Match3Position) {
    // no-op
  }
  ...
  private async swapPieces(pieceA: Match3Piece, pieceB: Match3Piece) {
    ...
    if (!valid) {
      ...
      if (DEBUG) {
        const specialTypeA = match3GetPieceType(this.match3.board.grid, positionA);
        console.log(
          '[Special] posA is special. typeA (before swap):',
          typeA,
          '| typeB (before swap):',
          typeB,
          '| specialType now at posA:',
          specialTypeA,
          '| swappedType passed:',
          typeA,
        );
      ...
      if (DEBUG) {
        const specialTypeB = match3GetPieceType(this.match3.board.grid, positionB);
        console.log(
          '[Special] posB is special. typeA (before swap):',
          typeA,
          '| typeB (before swap):',
          typeB,
          '| specialType now at posB:',
          specialTypeB,
          '| swappedType passed:',
          typeB,
        );
