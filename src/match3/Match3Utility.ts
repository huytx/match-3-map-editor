...
export function match3HasPossibleMoves(grid: Match3Grid, commonTypes: Match3Type[]): boolean {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      const type = grid[r][c];
      if (!commonTypes.includes(type)) continue;

      // Check swap with right neighbour
      if (c + 1 < columns && commonTypes.includes(grid[r][c + 1])) {
        const clone = match3CloneGrid(grid);
        match3SwapPieces(clone, { row: r, column: c }, { row: r, column: c + 1 });
        if (
          match3GetMatches(clone, [
            { row: r, column: c },
            { row: r, column: c + 1 },
          ]).length > 0
        ) {
      ...
      if (r + 1 < rows && commonTypes.includes(grid[r + 1][c])) {
        const clone = match3CloneGrid(grid);
        match3SwapPieces(clone, { row: r, column: c }, { row: r + 1, column: c });
        if (
          match3GetMatches(clone, [
            { row: r, column: c },
            { row: r + 1, column: c },
...
/**
 * Shuffle all common-type pieces in the grid in place, ensuring no pre-made matches
 * and at least one valid move exists after shuffling.
 * @param grid The grid to be shuffled (modified in place)
 * @param commonTypes The list of common (non-special) piece types
 * @returns The list of positions whose types were changed
 */
export function match3ShuffleGrid(grid: Match3Grid, commonTypes: Match3Type[]): Match3Position[] {
  // Collect all positions that hold a common-type piece
  const positions: Match3Position[] = [];
  const types: Match3Type[] = [];

  for (let r = 0; r < grid.length; r++) {
    for (let c = 0; c < grid[0].length; c++) {
      if (commonTypes.includes(grid[r][c])) {
        positions.push({ row: r, column: c });
        types.push(grid[r][c]);
      }
    }
  ...
  // Fisher-Yates shuffle, retry until the result has no pre-made matches and has valid moves
  let attempts = 0;
  let valid = false;
  ...
  while (!valid && attempts < 100) {
    ...
    // Test the shuffled arrangement on a clone before committing
    const testGrid = match3CloneGrid(grid);
    for (let i = 0; i < positions.length; i++) {
      testGrid[positions[i].row][positions[i].column] = types[i];
    }

    const noPreMadeMatches = match3GetMatches(testGrid).length === 0;
    const hasMoves = match3HasPossibleMoves(testGrid, commonTypes);
    valid = noPreMadeMatches && hasMoves;
    attempts++;
  }

  // Apply final arrangement to the real grid
  for (let i = 0; i < positions.length; i++) {
    grid[positions[i].row][positions[i].column] = types[i];
  }
