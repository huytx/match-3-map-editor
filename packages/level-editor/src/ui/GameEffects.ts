...
import { Match3, Match3OnMatchData, Match3OnMoveData, Match3OnPopData } from '@puzzling-potions/core';
import { Match3Piece } from '@puzzling-potions/core';
...
/** Minimal interface of the game screen needed by GameEffects */
interface GameEffectsOwner {
...
export class GameEffects extends Container {
  ...
  private game: GameEffectsOwner;

  constructor(game: GameEffectsOwner) {
    super();
    this.game = game;
  ...
  private async playPieceExplosion(piece: Match3Piece) {
    ...
    throttle('pieceExplosion', 100, () => this.playPopExplosion(position));
  ...
  public async playGridExplosion() {
    ...
    this.game.match3.board.pieces.forEach((piece: Match3Piece) => {
