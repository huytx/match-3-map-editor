...
/** Pixi Application instance, set via initNavigation() */
let _app: Application;

/** Must be called once after the Pixi Application is initialised */
export function initNavigation(pixiApp: Application) {
  _app = pixiApp;
}
...
class Navigation {
  ...
  private async addAndShowScreen(screen: AppScreen) {
    ...
    if (screen.update) {
      _app.ticker.add(screen.update, screen);
    }
  ...
  private async hideAndRemoveScreen(screen: AppScreen) {
    ...
    if (screen.update) {
      _app.ticker.remove(screen.update, screen);
    }
  ...
  /**
   * Immediately remove the current screen without playing its hide animation.
   * Use this when navigating away mid-session (e.g. editor back button) so
   * the hide animation doesn't play on the next showScreen() call.
   */
  public abandonCurrentScreen() {
    const screen = this.currentScreen;
    if (!screen) return;
    this.currentScreen = undefined;
    screen.interactiveChildren = false;
    if (screen.update) _app.ticker.remove(screen.update, screen);
    if (screen.parent) screen.parent.removeChild(screen);
    if (screen.reset) screen.reset();
    pool.giveBack(screen);
  }
