...
import { getGlobalTicker } from '../utils/ticker';
...
export class Match3Piece extends Container {
  ...
  public renderUpdate() {
    ...
    if (this.highlight.visible) {
      ...
      this.highlight.rotation += ticker.deltaTime * 0.03;
  ...
  public setIce(hp: 0 | 1 | 2 | 3) {
    ...
    const h = S / 2;
    // Solid fill — higher HP = more opaque
    const alpha = 0.15 + hp * 0.17; // HP1=0.32, HP2=0.49, HP3=0.66
    this.iceOverlay.roundRect(-h, -h, S, S, 6).fill({ color: 0x88ddff, alpha });
    // Cracks appear as HP decreases (ice breaking)
    if (hp <= 2) {
      this.iceOverlay
        .moveTo(-h * 0.2, -h * 0.7)
        .lineTo(h * 0.1, 0)
