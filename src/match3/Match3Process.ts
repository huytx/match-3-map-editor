...
export class Match3Process {
  ...
  private async processCheckpoint() {
    ...
    if (newMatches.length || emptySpaces.length) {
      ...
      if (this.match3.config.enableDeadlock) {
        ...
        // stop() must be called before onDeadlock so isProcessing() is false when the handler fires
        await this.stop();
  ...
  /** Shuffle all common-type pieces on the board when no valid moves remain */
  private async shuffleBoard() {
    const positions = match3ShuffleGrid(this.match3.board.grid, this.match3.board.commonTypes);
    // Respawn each shuffled piece with a spawn animation
    const animPromises = positions.map((pos) => {
      const type = match3GetPieceType(this.match3.board.grid, pos);
      return this.match3.board.spawnPiece(pos, type);
    });
    await Promise.all(animPromises);
  }
}
